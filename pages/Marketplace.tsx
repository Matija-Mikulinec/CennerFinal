...
import { CATEGORIES, MOCK_LISTINGS, MOCK_JOBS } from '../constants';
import { ServiceListing, JobPosting } from '../types';
        ...
        <div className="flex-grow">
          {mode === 'freelancers' ? (
            ...
            filteredServices.length > 0 ? (
              ...
              <EmptyState onReset={() => {setSearchTerm(''); setSelectedCategory('All'); setPriceRange(3000);}} />
            )
          ) : (
            /* Job Request Board */
...
const EmptyState: React.FC<{ onReset: () => void }> = ({ onReset }) => (
  <div className="text-center py-32 bg-brand-grey/20 rounded-[3rem] border-2 border-dashed border-white/5">
    <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-6 text-gray-600">
      <Search size={36} />
    </div>
    <h3 className="text-2xl font-black text-white mb-3">No Nodes Synchronized</h3>
    <p className="text-gray-500 font-medium max-w-sm mx-auto">We couldn't find any results matching your protocol parameters. Try expanding your search node.</p>
    <button
      onClick={onReset}
      className="mt-8 px-8 py-3 bg-brand-pink text-white font-black rounded-xl hover:scale-105 transition-all shadow-lg"
    >
      Reset All Protocols
    </button>
  </div>
